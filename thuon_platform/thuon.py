...
_REGISTRY: dict[str, dict] = {
	...
	'fleet_health_monitor': {
		...
		'deps': ['ai_engine'],
		'method': 'check',
		'description': 'Poll all fleet servers for CPU/RAM/disk/load health via SSH',
		'keywords': ['fleet', 'health', 'servers', 'cpu', 'memory', 'disk', 'monitoring'],
		'example': 't.fleet_health_monitor(hosts="all")',
	},
	'security_auditor': {
		...
		'deps': ['ai_engine'],
		'method': 'audit',
		'description': 'Audit servers for cryptominers, failed SSH, and open port exposure',
		'keywords': ['security', 'audit', 'miner', 'ssh', 'vulnerability', 'harden'],
		'example': 't.security_auditor(hosts="all")',
	},
	'service_status_checker': {
		...
		'deps': ['ai_engine'],
		'method': 'check',
		'description': 'Check systemd service status across the fleet',
		'keywords': ['service', 'systemd', 'status', 'running', 'failed', 'inactive'],
		'example': 't.service_status_checker(service="caddy", hosts="all")',
	},
	'log_analyzer': {
		...
		'deps': ['ai_engine'],
		'method': 'analyze',
		'description': 'Tail and AI-analyse logs from remote servers (journald or log files)',
		'keywords': ['logs', 'journal', 'errors', 'tail', 'analyse', 'debug'],
		'example': 't.log_analyzer(host="easf", source="journald:caddy")',
	},
	'service_manager': {
		...
		'deps': ['ai_engine'],
		'method': 'manage',
		'description': 'Start/stop/restart/reload systemd services on remote hosts (dry_run=True by default)',
		'keywords': ['restart', 'start', 'stop', 'service', 'systemctl', 'reload'],
		'example': 't.service_manager(action="restart", service="caddy", host="easf", dry_run=False)',
	},
	'deployment_runner': {
		...
		'deps': ['ai_engine'],
		'method': 'deploy',
		'description': 'Deploy to remote server: git pull + deploy script + systemctl reload (dry_run=True)',
		'keywords': ['deploy', 'git', 'pull', 'release', 'rollout', 'update'],
		'example': 't.deployment_runner(host="easf", repo_path="/opt/lindela", dry_run=False)',
	},
	'postgres_operator': {
		...
		'deps': ['ai_engine'],
		'method': 'operate',
		'description': 'PostgreSQL health, replication lag, dump, and bloat analysis via SSH',
		'keywords': ['postgres', 'database', 'replication', 'dump', 'backup', 'lag', 'bloat'],
		'example': 't.postgres_operator(action="replication_lag", host="db-main")',
	},
	'openbao_operator': {
		...
		'deps': ['ai_engine'],
		'method': 'operate',
		'description': 'Inspect OpenBao (Vault) seal status and get guided unseal instructions',
		'keywords': ['vault', 'openbao', 'unseal', 'secrets', 'sealed', 'token'],
		'example': 't.openbao_operator(action="seal_status")',
	},
	'backup_operator': {
		...
		'deps': ['ai_engine'],
		'method': 'operate',
		'description': 'Trigger pg_dump, verify integrity, and check backup disk usage',
		'keywords': ['backup', 'dump', 'restore', 'verify', 'pg_dump', 'disaster recovery'],
		'example': 't.backup_operator(action="list_dumps", host="db-main")',
	},
	'firewall_manager': {
		...
		'deps': ['ai_engine'],
		'method': 'manage',
		'description': 'Manage UFW firewall rules and audit open port exposure (dry_run=True)',
		'keywords': ['firewall', 'ufw', 'ports', 'rules', 'allow', 'deny', 'security'],
		'example': 't.firewall_manager(action="audit", host="ml")',
	},
	'incident_analyst': {
		...
		'deps': ['ai_engine'],
		'method': 'analyze',
		'description': 'Gather diagnostics from a failing server and produce root-cause analysis with remediation steps',
		'keywords': ['incident', 'outage', 'debug', 'root cause', 'triage', 'diagnose', 'down'],
		'example': 't.incident_analyst(host="easf", symptom="site returning 502")',
	},
	'linux_admin_assistant': {
		...
		'deps': ['ai_engine'],
		'method': 'ask',
		'description': 'Natural-language Linux admin: ask any sysadmin question, get analysis + recommended commands',
		'keywords': ['linux', 'admin', 'sysadmin', 'command', 'help', 'how to', 'server'],
		'example': 't.linux_admin_assistant(question="why is the search server slow?", execute_safe=True)',
	},
}
